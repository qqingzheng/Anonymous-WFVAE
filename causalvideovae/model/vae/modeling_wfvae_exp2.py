...
class Encoder(VideoBaseAE):
    ...
    def forward(self, coeffs):
        ...
        h = nonlinearity(h)
        h = self.conv_out(h)
        return h, (l1_coeffs, torch.tensor([0.], dtype=coeffs.dtype, device=coeffs.device))
...
class Decoder(VideoBaseAE):
    ...
    def forward(self, z):
        ...
        return h, (l1_coeffs, torch.tensor([0.], dtype=z.dtype, device=z.device))


@ModelRegistry.register("WFVAE_NOL3")
class WFVAEModel_NOL3(VideoBaseAE):
    ...
    def encode(self, x):
        ...
        if self.use_tiling:
            ...
            h, (l1, l2) = self.encoder(coeffs)
            if self.use_quant_layer:
                h = self.quant_conv(h)
            
        posterior = DiagonalGaussianDistribution(h)
        return posterior, (l1, l2)
    ...
    def forward(self, input, sample_posterior=True):
        posterior, (enc_l1, enc_l2) = self.encode(input)
        if sample_posterior:
            z = posterior.sample()
        else:
            z = posterior.mode()
        dec, (dec_l1, dec_l2) = self.decode(z)
        return dec, posterior, (enc_l1, dec_l1, enc_l2, dec_l2)

...
class WFVAEModelDiffusers(WFVAEModel):
    def encode(self, x: torch.Tensor) -> AutoencoderKLOutput:
        origin_encode_outputs = super().encode(x)
        return AutoencoderKLOutput(latent_dist=origin_encode_outputs[0])

    def decode(self, z: torch.Tensor) -> DecoderOutput:
        origin_decode_outputs = super().decode(z)
        return DecoderOutput(sample=origin_decode_outputs[0])
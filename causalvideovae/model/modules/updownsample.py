...
def custom_pad(x, value=0):
    pad_right, pad_bottom = (1, 1)
    right_pad = torch.full(
        (x.shape[0], x.shape[1], x.shape[2], pad_right),
        value,
        dtype=x.dtype,
        device=x.device,
    )
    bottom_pad = torch.full(
        (x.shape[0], x.shape[1], pad_bottom, x.shape[3] + pad_right),
        value,
        dtype=x.dtype,
        device=x.device,
    )
...
class Spatial2xTime2x3DDownsampleKeyFrame(Block):
    def __init__(self, in_channels, out_channels, mix_factor=2.0):
        super().__init__()
        self.key_frame_conv = Conv2d(
            in_channels, out_channels, kernel_size=3, stride=2, padding=0
        )
        self.conv = CausalConv3d(
    ...
    def forward(self, x):
        ...
        x = torch.nn.functional.pad(x, pad, mode="constant", value=0)
        alpha = torch.sigmoid(self.mix_factor)
        x_keyframes = torch.concat([x[:, :, :1], x[:, :, 1::2]], dim=2)
        x = self.key_frame_conv(x_keyframes) * alpha + self.conv(x) * (1 - alpha)
        return x

...
class LPIPSWithDiscriminator3D(nn.Module):
    ...
    def forward(
        ...
        t = inputs.shape[2]
        if optimizer_idx == 0:
            wl = HaarWaveletTransform3D().to(inputs.device, dtype=inputs.dtype)
            inputs_wl = wl(inputs)
            reconstructions_wl = wl(reconstructions)
            ...
            # subbands_loss = torch.sum(l1(inputs_wl[:, 12:15], reconstructions_wl[:, 12:15])) / bs # + \
            # torch.sum(l1(inputs_wl[:, 3:9], reconstructions_wl[:, 3:9])) / bs
